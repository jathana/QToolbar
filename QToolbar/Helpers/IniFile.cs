...
namespace QToolbar
{
   ...
   public class IniFile
   {
      ...
      public string[] GetAllValues(string section, string key)
      {
         ...
         while (true)
         {
            ...
            if (!dictionary.TryGetValue(key3, out value))
               break;
            values.Add(GetClearedFromXXXStrings(value));
         }
      ...
      /// <returns>The array of values, or null if none found.</returns>
      /// <seealso cref="GetValue"/>
      public Dictionary<string,string> GetSectionPairs(string section)
      {
         ...
         foreach(string key in dictionary.Keys)
         {
            ...
            {
               retVal.Add(key.Replace(section,""), GetClearedFromXXXStrings(dictionary[key]));
            }
         }
         return retVal;
      }
      ...
      private string GetClearedFromXXXStrings(string val)
      {
         string[] strs = Options.OptionsInstance.ClearValuesFromCFs.Split(',');
         string retVal = val;
